...
impl ShopOverlay {
    pub fn new(card_used: usize) -> Self {
        return Self{card_used, cur_selected: 0}
    }

    fn get_available_cards(&self, _state: &GameState) -> Vec<CardType>{
        return vec!(CardType::Build(TowerType::Archers), CardType::Build(TowerType::Cannon), CardType::Shop, CardType::SellTower, CardType::DamageEnemy);
    }
...
impl OverlayState for ShopOverlay {
    ...
    fn key_down_event(
        ...
        match keycode {
            Keycode::Up => self.cur_selected = add_mod(self.cur_selected, -1, self.get_available_cards(state).len()),
            Keycode::Down => self.cur_selected = add_mod(self.cur_selected, 1, self.get_available_cards(state).len()),
