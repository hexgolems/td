...
enum CursorMode {
    Build{x: usize, y: usize, t: TowerType, valid: bool},
}
...
impl CursorMode{
    fn update(&mut self, state: &GameState){
        match self {
            Build{x,y, ref mut valid, ..} => *valid = state.map.is_buildable(*x,*y) && state.towers.is_buildable(*x, *y)
        }
    }
    ...
    pub fn up(&self, state: &GameState) -> Self {
        ...
        match res{
            Build{x, ref mut y, ..} => if *y>0 && state.map.inbounds(x,*y-1) { *y-=1; }
        }
        res.update(state);
        return res
    }
    pub fn down(&self, state: &GameState) -> Self{
        let mut res = self.clone();
        match res{
            Build{x,ref mut y, .. } => if state.map.inbounds(x,*y+1) { *y+=1; }
        }
        res.update(state);
        return res;
    }
    pub fn left(&self, state: &GameState) -> Self{
        let mut res = self.clone();
        match res{
            Build{ref mut x,y, ..} => if *x>0 && state.map.inbounds(*x-1,y) { *x -=1; }
        }
        res.update(state);
        return res;
    }
    pub fn right(&self, state: &GameState) -> Self{
        let mut res = self.clone();
        match res{
            Build{ref mut x,y, ..} => if state.map.inbounds(*x+1,y) { *x+=1; }
        }
        res.update(state);
        return res;
    }
...
impl Gui {
    pub fn new() -> Self {
        return Self {
            cursor_state: CursorMode::Build{x:0, y:0, t: TowerType::Cannon, valid: false},
        };
    }

    fn draw_map_cursor(&self,x: usize,y: usize, imgs: &Imgs, ctx: &mut Context) -> GameResult<()>{
        graphics::draw_ex(
            ctx,
    ...
    fn draw_build_preview(&self,x: usize,y: usize, t: TowerType, valid: bool, imgs: &Imgs, ctx: &mut Context) -> GameResult<()>{
        let color = if valid { 
    ...
    pub fn key_down(state: &mut GameState, keycode: Keycode, _keymod: Mod, _repeat: bool) {
        ...
        if keycode == Keycode::Space {
            match state.gui.cursor_state {
                CursorMode::Build{x, y, t: TowerType::Cannon, valid: true} => state.towers.spawn( Tower::new((x,y), 1.0, 1.0, 0.5)),
                _ => {},
            }
        }
    }
